...
class KANLinearHead(nn.Module):
    def __init__(self, in_features, out_features, grid_size=5, spline_order=3,
                 scale_base=1.0, scale_spline=1.0, base_activation=nn.SiLU, grid_range=[-1, 1]):
        super().__init__()
    ...
    def b_splines(self, x):
        ...
        for p in range(1, self.spline_order + 1):
            bases = (
                (x - grid[:, :-(p + 1)]) / (grid[:, p:-1] - grid[:, :-(p + 1)] + 1e-10) * bases[:, :, :-1]
            ) + (
                (grid[:, p + 1:] - x) / (grid[:, p + 1:] - grid[:, 1:(-p)] + 1e-10) * bases[:, :, 1:]
            )
    ...
    def forward_with_basis(self, x):
        ...
        return base_output + spline_output, bases


class ResidualKANHead(nn.Module):
    """
    output = conv_output + alpha * kan_output

    alpha is initialized to 0, so early training is equivalent to a pure Conv head and does not hurt performance.
    alpha is learnable, allowing KAN to gradually contribute residual predictions.
    KAN basis functions directly contribute to prediction, making entropy a meaningful uncertainty signal.
    """
    def __init__(self, in_features, num_classes, grid_size=3, spline_order=2):
        super().__init__()
        self.in_features = in_features
    ...
    def forward(self, x, return_spline_basis=False):
        B, C, H, W = x.shape
        ...
        if return_spline_basis:
            kan_out, bases = self.kan.forward_with_basis(x_flat)
            kan_out = kan_out.reshape(B, H, W, self.num_classes).permute(0, 3, 1, 2)
            bases = bases.reshape(B, H, W, self.in_features, -1)
            out = conv_out + self.alpha * kan_out
            return out, bases
        else:
...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        # Residual KAN head
        self.final = ResidualKANHead(embed_dims[0] // 8, num_classes)
